...
name: debug
description:
  Investigate stuck runs and execution failures by tracing Jazzband and Codex
...
- Primary runtime log: `log/jazzband.log`
  - Default comes from `JazzbandElixir.LogFile` (`log/jazzband.log`).
  - Includes orchestrator, agent runner, and Codex app-server lifecycle logs.
- Rotated runtime logs: `log/jazzband.log*`
...
rg -n "issue_identifier=MT-625" log/jazzband.log*

# 2) If needed, narrow by Linear UUID
rg -n "issue_id=<linear-uuid>" log/jazzband.log*

# 3) Pull session IDs seen for that ticket
rg -o "session_id=[^ ;]+" log/jazzband.log* | sort -u

# 4) Trace one session end-to-end
rg -n "session_id=<thread>-<turn>" log/jazzband.log*

# 5) Focus on stuck/retry signals
rg -n "Issue stalled|scheduling retry|turn_timeout|turn_failed|Codex session failed|Codex session ended with error" log/jazzband.log*
```
...
In Jazzband, Codex session diagnostics are emitted into `log/jazzband.log` and
...
- Check rotated logs (`log/jazzband.log*`) before concluding data is missing.
