    ...
    def test_new_project_init_prints_publish_command_without_running_it(self):
        for runner in ("claude_code", "codex"):
            with self.subTest(runner=runner), tempfile.TemporaryDirectory() as temp_dir:
                ...
                stdout = StringIO()
                with (
                    patch("symphony.cli.setup_environment_checks", return_value=[]),
                    patch("symphony.cli._automated_setup_failures", return_value=[]),
                    patch("symphony.cli.subprocess.run") as command,
