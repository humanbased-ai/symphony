...
``jazzband.acceptance`` and Jazzband's PR polling loop. ``runtime.py`` calls
...
def gather_convergence_inputs(
    *,
    ...
    inputs = ConvergenceInputs(
        ...
        # cr-autofix branch name is not codified in Jazzband's config). When
...
class ClaudeCodeJudgeRunner:
    ...
    Distinct from ``jazzband.agents.claude_code.ClaudeCodeRunner``: the judge
    ...
    async def judge(self, system_prompt: str, user_prompt: str) -> str | None:
        ...
        with tempfile.TemporaryDirectory(prefix="jazzband-acceptance-") as cwd:
            try:
...
def render_verdict_comment(
    ...
    *,
    ...
    The first line is the ``JAZZBAND_BOT_MARKER`` so the poller's own filtering
    ...
    from the PR thread alone without digging into Jazzband logs.
    """
    ...
    if merged:
        ...
        if auto_merge_skip_reason:
            ...
            lines.append(
                "**Jazzband will not auto-merge this PR.** A human reviewer "
