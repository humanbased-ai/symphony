...
RUN_METADATA_SUFFIX = ".jazzband-workspace.json"
...
class WorkspaceManager:
    ...
    * Git mode (`workspace.repo_url` configured): maintain a bare clone at
      ...
      removes the worktree and deletes the branch Jazzband recorded at create
    ...
    * Plain mode (no `repo_url`): create an empty per-run directory at the same
    ...
    async def sweep_stale_worktrees(self) -> int:
        ...
        Jazzband does not persist its in-memory running set across restarts, so
