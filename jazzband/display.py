"""Live terminal dashboard for Jazzband runtime state."""
...
if TYPE_CHECKING:
    from jazzband.agents.base import AgentEvent
    from jazzband.orchestrator import OrchestratorState
    from jazzband.runtime import RuntimeTickResult
...
class LiveDashboard:
    """Rich live terminal dashboard showing per-issue Jazzband state."""
    ...
    def _render(self) -> Group:
        ...
        from jazzband import __version__  # noqa: PLC0415
        panel = Panel(
            Group(stats, Text(""), tbl, Rule(style="dim #1a1a1a"), footer),
            title=f"[bold white]Jazzband[/bold white] [dim]{__version__}[/dim]",
            title_align="left",
            border_style="#2a2a2a",
        )
