...
# Lowered from 10 to 5 (IN-628): the Jazzband↔Crosscheck fix loop rarely needs
...
# Best-effort cross-daemon lock so two Jazzband daemons don't dispatch fix agents
# for the same PR at once (IN-628). Applied as a GitHub label while a fix agent runs.
DEFAULT_PR_LOCK_LABEL = "jazzband:working"
    ...
    def from_mapping(
        ...
        *,
        ...
        configured_root = _string_value(workspace.get("root")) or str(
            Path(tempfile.gettempdir()) / "jazzband_workspaces"
        )
...
class AcceptanceConfig:
    ...
    - ``review_source: none``       no external reviewer; converge on Jazzband
...
class VerifyflowConfig:
    ...
    head, Jazzband spawns ``vf step --pr <url>`` once per head SHA. VerifyFlow
    ...
    delivery-report comment on the PR. Jazzband only logs the JSON result —
...
class WorkflowConfig:
    ...
    workspace: WorkspaceConfig = field(
        default_factory=lambda: WorkspaceConfig(Path(tempfile.gettempdir()) / "jazzband_workspaces")
    )
