...
from jazzband.acceptance import verdict_oscillating
...
from jazzband.feedback import ClassifyError, FeedbackSignal, classify_feedback
from jazzband.tracker.models import Issue
from jazzband.workflow import WorkflowDefinition, render_prompt
...
# HTML comment prefix injected into every PR comment Jazzband posts.
# Filtering is done on this marker so that a user whose personal GitHub token
# is also the Jazzband bot token can still have their own comments picked up.
JAZZBAND_BOT_MARKER = "<!-- jazzband -->"
...
class JazzbandRuntime:
    """Offline-testable runtime coordinator for one Jazzband poll tick."""
    ...
    async def _poll_pr_for_branch(self, branch: str, issue: Issue) -> None:
        ...
        for comment in review_comments:
            ...
            if body.startswith(JAZZBAND_BOT_MARKER):
                continue
...
4. Post a comment on the PR summarizing what you changed. The body MUST start with
   `<!-- jazzband -->` on its own first line so the automation skips your comment on
   the next polling cycle (without this marker your comment will trigger another run):
     gh pr comment {pr_number} --body $'<!-- jazzband -->\\nI addressed the feedback by ...'
...
5. Post a comment on the PR confirming the conflicts are resolved. The body MUST start with
   `<!-- jazzband -->` on its own first line:
