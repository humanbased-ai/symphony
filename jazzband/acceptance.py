...
- **crosscheck branch** — an external AI reviewer (``@motivation-labs/crosscheck``)
  ...
  period has elapsed.
- **silent branch** — no external reviewer; we converge when Jazzband itself has
...
def evaluate_convergence(inputs: ConvergenceInputs) -> ConvergenceResult:
    ...
    return ConvergenceResult(True, "silent", "Jazzband quiesced; CI green; converged")
...
def verdict_oscillating(
    ...
    *,
    ...
    mean the loop is thrashing rather than progressing, and Jazzband should stop
...
You are Jazzband's acceptance judge. You make the final call on whether a
