...
**[PR #160](https://github.com/humanbased-ai/jazzband/pull/160)**: feat(pr-loop): single-writer lock + oscillation guard, lower turn budget (IN-628)

## Linear

- Issue: IN-628 — Pipeline convergence: stop the Jazzband↔Crosscheck↔VerifyFlow repair loop from oscillating

## Summary

- Jazzband's slice of IN-628. Two fixes for the non-converging review/fix loop seen on IN-625 PR #28.
- **P0 — single-writer PR lock**: `GitHubClient` gains `list_pr_labels`/`add_pr_labels`/`remove_pr_label`; `_handle_pr_feedback` claims the PR with a `jazzband:working` label before dispatching a fix agent and releases it in a `finally`. A second dispatch that sees the label held skips the PR that tick without consuming a turn. Configurable via `github.pr_lock_enabled` / `pr_lock_label`.
...
- Selected solution: severity gate lives in Crosscheck (#208, the core fix); Jazzband adds the orchestration-side backstops — an oscillation circuit-breaker and a best-effort PR lock.
...
- Follow-up work: the heavier half of P1 (incremental review + findings ledger) is deferred to a follow-up; a label lock cannot block a human's manual push (only a second Jazzband dispatch).
...
**[PR #159](https://github.com/humanbased-ai/jazzband/pull/159)**: feat(verifyflow): auto-merge gate on accept verdict (IN-609)
...
- VerifyFlow becomes the delivery merge judge; Jazzband's LLM acceptance judge stays disabled (they do not both run).
...
- Not run: live `jazzband run` end-to-end (requires the daemon + a real PR); covered by the unit tests' injected spawn/GitHub fakes.
...
**[PR #154](https://github.com/humanbased-ai/jazzband/pull/154)**: Add `jazzband info` command to print environment diagnostics

## Linear

- Issue: https://linear.app/inductive-network/issue/IN-545/add-jazzband-info-command-to-print-environment-diagnostics

## Summary

- Add `jazzband info` (alias `sy info`) printing read-only environment diagnostics.
- **Version & platform**: `Jazzband <version> — Python <pyver> on <platform>`.
...
**[PR #158](https://github.com/humanbased-ai/jazzband/pull/158)**: fix(acceptance): parse modern crosscheck comment formats (IN-572 live-run finding)
...
**[PR #157](https://github.com/humanbased-ai/jazzband/pull/157)**: feat(verifyflow): ci_green trigger for repos without Crosscheck (IN-570)
...
**[PR #155](https://github.com/humanbased-ai/jazzband/pull/155)**: fix(onboarding): default acceptance gate to off in init/onboard
...
- `jazzband init` / `jazzband onboard` now ship the acceptance gate **disabled by default**: `InitConfig.acceptance_enabled` defaults to `False`, automated mode no longer auto-enables the gate, and the interactive prompt defaults to `[y/N]`.
...
- Not run: interactive `jazzband init` end-to-end (prompt-path behavior covered by unit tests).
...
**[PR #153](https://github.com/humanbased-ai/jazzband/pull/153)**: fix(acceptance): gate bounce-back behind opt-in flag (default off)
...
- ``jazzband init`` writes ``bounce_back_on_fail: false`` explicitly so users see the knob and know it's the off-by-default opt-in.
...
- **Why write the false explicitly in ``jazzband init``**: same reason ``auto_merge: false`` is written explicitly — the file should disclose every irreversible automation knob, not hide them behind library defaults. Users discover what's tunable by reading the generated file.
...
**[PR #152](https://github.com/humanbased-ai/jazzband/pull/152)**: fix(acceptance): hold auto mode for crosscheck before falling through
...
- ``jazzband init`` writes ``crosscheck_wait_seconds`` into the generated WORKFLOW.md so the knob is discoverable.
...
**[PR #147](https://github.com/humanbased-ai/jazzband/pull/147)**: feat(onboarding): scaffold disabled-by-default acceptance block

## Linear

- Issue: N/A — onboarding ergonomics follow-up to the acceptance gate shipped in PR #138/#145/#146. Surface the feature inside `jazzband init` rather than burying it in docs.

## Summary

- `jazzband init` / `jazzband onboard` now ask **"Enable acceptance gate? [Y/n]"** after the GitHub repo step, when github is configured and the run is interactive. Pressing Enter keeps it on; "n" turns it off.
...
- Defaults sourced from `jazzband.config.DEFAULT_ACCEPTANCE_*` so scaffold and runtime parser stay in sync.
...
- `uv run jazzband init --help` shows `--acceptance, --no-acceptance` with the right default-behavior text.
- `uv run jazzband --check` on the existing `WORKFLOW.md` still parses cleanly (no schema break for projects without an `acceptance:` block).
- Not run: live `jazzband onboard` interactive flow end-to-end — covered indirectly by `test_generate_workflow_enables_acceptance_by_default_with_github` and `test_generate_workflow_omits_acceptance_block_when_opted_out`.

## UI Evidence

- CLI flag visible in `jazzband init --help` output. No graphical UI surface.
...
**[PR #150](https://github.com/humanbased-ai/jazzband/pull/150)**: fix(github): dedup check runs by name in get_pr_failed_check_runs
...
**[PR #149](https://github.com/humanbased-ai/jazzband/pull/149)**: Update CHANGELOG.md and README.md for acceptance gate
...
Docs-only sync for the acceptance gate that shipped across #138 (convergence core), #145 (judge prompt), #146 (runtime wiring), and #148 (Phase 2 auto-merge). No code under `jazzband/` was touched.
...
- Full gates: not applicable; docs-only change with no code under `jazzband/` touched, so no compile/test gate runs.
...
- Reviewer / agent requested: Jazzband automation / repo maintainers.
...
**[PR #148](https://github.com/humanbased-ai/jazzband/pull/148)**: feat(acceptance): Phase 2 — gated auto-merge for the safest PRs
...
- ``GitHubClient.merge_pr(pr_number, *, sha=None, merge_method="squash", commit_title=None)`` — squash-merges via `PUT /repos/{owner}/{repo}/pulls/{n}/merge`. The optional ``sha`` is forwarded to GitHub's ``required_head`` check so a commit pushed after the judge ran blocks the merge instead of slipping past Jazzband.
...
- **Default off** rationale: auto-merge is the only Jazzband feature that irreversibly touches ``main``. Flipping it on must remain an explicit edit, not a side-effect of enabling acceptance. The onboarding scaffold from PR #147 already writes ``auto_merge: false`` for this reason.
...
**[PR #145](https://github.com/humanbased-ai/jazzband/pull/145)**: feat(acceptance): judge system prompt with scope-boundary tests
...
- Add `ACCEPTANCE_JUDGE_SYSTEM_PROMPT` in `jazzband/acceptance.py` — the system prompt the runner must pass verbatim when it dispatches the one-shot acceptance judge.
...
**[PR #142](https://github.com/humanbased-ai/jazzband/pull/142)**: chore: default codex-safe preset to 3 concurrent agents
...
- Bump the default `codex-safe` onboarding preset from `max_concurrent_agents=1` to `3` (`jazzband/onboarding.py`).
...
**[PR #136](https://github.com/humanbased-ai/jazzband/pull/136)**: feat(in-390): run manifest collection and report generation
...
- `jazzband/manifest.py` — `ManifestWriter` appends one JSON line per dispatch to `~/.jazzband/runs/<project-slug>/<run-id>/manifest.jsonl`; run ID persists across daemon restarts so data isn't fragmented by restarts; `--new-run` starts a fresh run
- `jazzband/report.py` — reads manifest, calls `claude` CLI, writes `report.md`; token costs and counterfactual (no-cache) cost are computed from manifest fields
...
- `cli.py` — wires `ManifestWriter` into `create_runtime` using project slug from config; automatically generates report on daemon stop without prompting; adds `jazzband report` command; adds `--new-run` flag
...
- Storage at `~/.jazzband/runs/<project-slug>/<run-id>/` — project-isolated
...
- `python -c "import jazzband.manifest; import jazzband.report; import jazzband.cli"` passes
- `jazzband report --help` shows correct options
- `jazzband --help` shows `jazzband report` in epilog
...
**[PR #135](https://github.com/humanbased-ai/jazzband/pull/135)**: feat(runtime): auto-detect PR merge conflicts and dispatch resolution agent
...
- Alternatives considered: GitHub Merge Queue — only handles the merge side, not the agent context divergence problem; rejected in favor of Jazzband-native resolution
- `max_pr_turns` budget is shared with regular feedback turns to bound total agent iterations per PR

## Validation

- Targeted checks: `python -c "import jazzband.runtime"` passes; logic reviewed manually
