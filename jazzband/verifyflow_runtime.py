...
a PR, Jazzband spawns ``vf step --pr <url>`` exactly once for that head SHA.
...
posts/updates its own idempotent delivery-report comment on the PR. Jazzband
...
acceptance judge = off. See verifyflow's ``docs/jazzband-integration-surface.md``
...
def _render_gate_comment(verdict: str | None, *, merged: bool, reason: str | None) -> str:
    ...
    if merged:
        return (
            ...
            "squash-merged by Jazzband. See VerifyFlow's delivery-report comment for evidence."
        )
    return (
        "<!-- verifyflow:merge-gate -->\n"
