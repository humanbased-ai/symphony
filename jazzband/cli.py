...
from jazzband.config import ConfigError, WorkflowConfig
from jazzband.github.client import GitHubClient
from jazzband.http_server import StatusAPI, WebhookAPI
...
from jazzband.onboarding_tutorial import run_init_tutorial_once
from jazzband.runtime import RuntimeTickResult, JazzbandRuntime
from jazzband.tracker.linear import LinearClient, LinearClientError
from jazzband.workflow import WorkflowError, load_workflow
from jazzband.workflow import EffectiveWorkflow, WorkflowReloader
from jazzband.workspace import WorkspaceManager
...
query JazzbandStarterTeams {
  viewer { teams(first: 10) { nodes { id name } } }
}
""".strip()

_STARTER_FIND_PROJECT_QUERY = """
query JazzbandStarterFindProject($name: String!) {
...
query JazzbandStarterStates($teamId: ID!) {
  workflowStates(
...
mutation JazzbandStarterCreateProject($name: String!, $teamIds: [String!]!) {
...
query JazzbandStarterFindIssue($projectId: ID!, $title: String!) {
...
mutation JazzbandStarterCreateIssue(
...
class _ColoredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Strip the "jazzband." package prefix so names stay compact
        name = record.name.removeprefix("jazzband.")
...
def _cli_name() -> str:
    """Return the name used to invoke this CLI (e.g. 'jazzband' or 'jz')."""
    if not sys.argv:
        return "jazzband"
...
class StartupError(RuntimeError):
    """Raised when Jazzband cannot start with the requested configuration."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=cli,
        description="Run the Jazzband CLI MVP orchestrator for a repository WORKFLOW.md.",
        epilog=(
    ...
    parser.add_argument(
        "--logs-root",
        default="./log",
        help="Directory for Jazzband runtime logs. Defaults to ./log.",
    )
    parser.add_argument(
        "--check",
        action="store_true",
...
def build_run_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run the Jazzband orchestrator for a repository WORKFLOW.md."
    return parser
...
def build_init_parser(prog: str | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--linear-api-key",
        help="Store this Linear API key in the local Jazzband credential file.",
    )
    parser.add_argument(
        "--credentials-path",
...
def build_info_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Print Jazzband version, platform, and workflow diagnostics.",
    )
    _add_version_argument(parser)
    parser.add_argument(
        "workflow_path",
...
def create_runtime(context: StartupContext, *, new_run: bool = False) -> JazzbandRuntime:
    from jazzband.manifest import ManifestWriter, get_or_create_run_id  # noqa: PLC0415
...
def create_status_api(runtime: JazzbandRuntime) -> StatusAPI:
...
async def run_once(runtime: JazzbandRuntime) -> RuntimeTickResult:
...
async def _startup_workspace_sweep(workspace_manager: WorkspaceManager) -> None:
    ...
    Jazzband does not persist its in-memory running set across restarts, so any
...
def _maybe_create_dashboard() -> "Any | None":
    ...
    if not sys.stdout.isatty():
        return None
    try:
        from jazzband.display import LiveDashboard  # noqa: PLC0415
...
async def run_daemon(
    ...
    *,
    ...
    if wh.url and wh.team_id and wh.secret:
        from jazzband.tracker.webhooks import WebhookRegistrar, WebhookRegistrarError  # noqa: PLC0415
        try:
...
def apply_runtime_workflow(runtime: JazzbandRuntime, effective: EffectiveWorkflow) -> None:
...
def _do_generate_report(manifest_path: Any, run_id: str) -> None:
    from jazzband.report import generate_report  # noqa: PLC0415
    try:
        generate_report(manifest_path, run_id)
    except Exception as exc:  # noqa: BLE001
        print(f"[jazzband] Report generation failed: {exc}", file=sys.stderr)


def report_main(argv: Sequence[str] | None = None) -> int:
    from jazzband.manifest import list_runs, JAZZBAND_HOME  # noqa: PLC0415
    from jazzband.report import generate_report  # noqa: PLC0415

    parser = argparse.ArgumentParser(prog="jazzband report", description="Generate a run analysis report")
    ...
    runs_root = JAZZBAND_HOME / "runs"
    if not runs_root.exists():
        print("jazzband: no runs found — start a daemon first", file=sys.stderr)
        return 1
    ...
    if not slugs:
        print("jazzband: no runs found", file=sys.stderr)
        return 1
    ...
    if not all_runs:
        print("jazzband: no manifest files found", file=sys.stderr)
        return 1

    if args.run_id:
        match = [(s, r, p) for s, r, p in all_runs if r == args.run_id]
        if not match:
            print(f"jazzband: run {args.run_id!r} not found", file=sys.stderr)
            return 1
    ...
    try:
        ...
        print(f"jazzband: {exc}", file=sys.stderr)
        return 1
...
def cache_main(argv: Sequence[str] | None = None) -> int:
    from jazzband.cache.store import CacheStateStore  # noqa: PLC0415

    parser = argparse.ArgumentParser(prog="jazzband cache", description="Prompt cache management")
    ...
    if args.cache_command == "status":
        ...
        state = store.load()
        if not state:
            print("No cache state found. Run Jazzband first to populate.")
            return 0
...
def changelog_main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="jazzband changelog", description="Show version history")
    parser.add_argument("-n", metavar="N", type=int, default=None, help="show only the last N versions")
    args = parser.parse_args(argv)

    try:
        ref = pkg_resources.files("jazzband").joinpath("CHANGELOG.md")
        content = ref.read_text(encoding="utf-8")
    except Exception:
        p = Path(__file__).parent.parent / "CHANGELOG.md"
        if not p.exists():
            print("jazzband: changelog not found", file=sys.stderr)
            return 1
...
def run_with_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    ...
    try:
        ...
        if "missing_workflow_file" in str(exc) and sys.stdin.isatty():
            ...
            try:
                ...
                parser.exit(2, f"jazzband: {exc2}\n")
                return 1
        else:
            parser.exit(2, f"jazzband: {exc}\n")
    ...
    if not port_ok:
        parser.exit(
            ...
            f"{_dim('Tip: each jazzband process needs its own port. '
                    'Run multiple projects with different --port values:')}\n"
            f"  jazzband run project-a/WORKFLOW.md --port {context.port + 1}\n"
            f"  jazzband run project-b/WORKFLOW.md --port {context.port + 2}\n",
        )

    project_slug = context.config.tracker.project_slug or "unknown"
    poll_interval_s = context.config.polling.interval_ms // 1000
    print(
        f"{_bold('Jazzband')} {_dim(__version__)}  |  "
...
def onboard_main(argv: Sequence[str] | None = None) -> int:
    ...
    if workflow_path.exists() and not args.overwrite:
        ...
        print()
        print("Fix options:")
        print(f"  Credentials only : jazzband onboard --linear-api-key lin_api_... --github-token ghp_...")
        print(f"  Regenerate config: jazzband onboard --overwrite")
        print(f"  Full details     : jazzband doctor {workflow_path}")
        parser.exit(2, "")
    ...
    *,
    ...
    try:
        ...
        if linear_token is None and not automated:
            ...
            print("  Jazzband uses this key to poll issues and post progress comments.")
        ...
        if args.acceptance_enabled is not None:
            ...
            print("  the PR. Phase 1 always escalates to a human — Jazzband will")
        ...
        parser.exit(2, f"jazzband {command_name}: {exc}\n")
    ...
    print(_dim(
        ...
        "Stop a process with Ctrl-C, or find its PID with: ps aux | grep 'jazzband run'"
    ))
...
def _offer_tutorial(args: argparse.Namespace) -> None:
    """Prompt to show the Jazzband intro tutorial; only runs in interactive TTY mode."""
    ...
    if mode == "automated":
        return
    try:
        answer = input("\nShow a quick intro to Jazzband? [Y/n] ").strip().lower()
...
def _offer_starter_mission(
    ...
    *,
    ...
    from jazzband.onboarding_tutorial import should_show_starter_mission  # noqa: PLC0415

    if not should_show_starter_mission(environ=environ):
        return

    print("\n★  Starter Mission (optional, runs once)")
    print("   Jazzband creates a sample Linear project with 1 ticket and dispatches")
...
def _run_starter_mission(
    ...
    *,
    ...
    try:
        ...
        body = _gql(_STARTER_FIND_PROJECT_QUERY, {"name": "jazzband-hello-world"})
        existing = ((body.get("data") or {}).get("projects") or {}).get("nodes") or []
        if existing:
            project = existing[0]
            project_id: str = project["id"]
            project_slug: str = project.get("slugId") or "jazzband-hello-world"
            print(f"  Using existing project (slug: {project_slug})")
        else:
            print("  Creating Linear project: jazzband-hello-world …")
            body = _gql(
                _STARTER_CREATE_PROJECT_MUTATION,
                {"name": "jazzband-hello-world", "teamIds": [team_id]},
            )
            ...
            project_slug = project.get("slugId") or "jazzband-hello-world"
        ...
        demo_dir = Path("jazzband-hello-world")
        ...
        # Step 7: record done
        from jazzband.onboarding_tutorial import record_starter_mission_done  # noqa: PLC0415
        ...
        if github_source:
            ...
            checks.append(
                (
                    False,
                    "github auth",
                    "not found — run: gh auth login, run jazzband init --github-token, or set GITHUB_TOKEN",
                )
            )
    else:
...
def info_main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"Jazzband {info['version']} — Python {info['python']} on {info['platform']}")
...
def webhooks_main(argv: Sequence[str] | None = None) -> int:
    ...
    if not api_key:
        parser.exit(2, "jazzband webhooks: LINEAR_API_KEY not set and --api-key not provided\n")
    ...
    if args.webhooks_command == "register":
        ...
        if not secret:
            parser.exit(2, "jazzband webhooks register: --secret or LINEAR_WEBHOOK_SECRET required\n")
        try:
            ...
            parser.exit(2, f"jazzband webhooks register: {exc}\n")
        return 0
    ...
    if args.webhooks_command == "list":
        try:
            ...
            parser.exit(2, f"jazzband webhooks list: {exc}\n")
        return 0
...
def _check_port_available(port: int) -> tuple[bool, str]:
    ...
    try:
        ...
        return (
            False,
            f"port {port} already in use — another jazzband process may be running. "
...
def _check_linear_key_valid(token: str, endpoint: str = "https://api.linear.app/graphql") -> tuple[bool, str]:
    ...
    try:
        with _req.urlopen(request, timeout=10) as resp:
            ...
            if not viewer:
                return False, "invalid key — check LINEAR_API_KEY or run: jazzband init --linear-api-key"
...
def _check_workspace_root(path: Path) -> tuple[bool, str]:
    try:
        ...
        probe = path / ".jazzband-write-check"
        probe.write_text("ok\n", encoding="utf-8")
        probe.unlink()
...
def project_main(argv: Sequence[str] | None = None) -> int:
    ...
    # --- detect running jazzband processes ---
...
def _detect_running_workflow_paths() -> set[str]:
    """Return set of absolute WORKFLOW.md paths found in running jazzband processes."""
    try:
        result = subprocess.run(["ps", "aux"], capture_output=True, text=True, timeout=5)
        paths: set[str] = set()
        for line in result.stdout.splitlines():
            if ("jazzband" in line or "sy " in line) and "run" in line:
                pid_m = re.match(r'\S+\s+(\d+)', line)
                pid = pid_m.group(1) if pid_m else None
                m = re.search(r'(?:jazzband|sy)\s+run\s+(\S+\.md)', line)
...
def _print_project_table(
    ...
    print(legend)
    print(
        _dim("Stop:  ") + _cyan("Ctrl-C") + _dim("  ·  kill <PID>  ·  pkill -f 'jazzband run'")
    )
    print(_dim("PID:   ps aux | grep 'jazzband run'"))
