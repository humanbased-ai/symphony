...
def _build_prompt(entries: list[dict], run_id: str) -> str:
    ...
    return f"""You are generating a Jazzband run analysis report for run: {run_id}
...
def generate_report(
    ...
    *,
    ...
    if not quiet:
        print("[jazzband] Generating run analysis...", file=sys.stderr, flush=True)
    ...
    if not quiet:
        print(f"[jazzband] Report written → {out_path}", file=sys.stderr, flush=True)
